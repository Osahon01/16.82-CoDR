...
class Takeoff_model:
    def __init__(self, T_W_takeoff, W_S, W, P_shaft_TO, CLTO, CDTO, CD0, AR, e):
        ...
        self.rho_SL = 1.225  # Air density at sea level (kg/m^3)
        self.rho_cruise = 0.8 # Air density at cruise (kg/m^3) - CURRENTLY ASSUMING 3000m CRUISE
    ...
    def get_A_prop(self, v):
        # Calculate cruise thrust using propeller power and efficiency
        A_prop = self.T**3/(self.P_shaft_TO**2 * self.rho_SL) * 1 / (self.eta_v * self.eta_add)
        return A_prop
    ...
    def takeoff_distance(self):
        ...
        def equations(t, y):
            x, x_dot = y
            d2x_dt2 = self.g * (self.T_W_takeoff - 0.5 * self.rho_SL * self.S / self.W * self.CDTO * x_dot**2)
